...
class RedisHash
{
     ...
     * @return bool
     */
    public function set($field,$value="",$chain=false){

        $res = '';
        switch(true) {
            //判断为空
            case (is_array($field)&&empty($value)):
                $res = $this->setAll($field);
                break;
            case  (is_object($field)&&empty($value)):
                $res = $this->setAll($field->toArray());
                break;
    ...
    public function deleteByKey($key1=null, $key2 = null, $key3 = null ) {
        ...
        $key1 = $this->redisHash[$this->sn]->table.":".$key1;
        return $this->redisHash[$this->sn]->del( $key1, $key2 , $key3 );
    }

    public function clear($pattern="") {
        if(!empty($pattern)){
            return $this->redisHash[$this->sn]->del($this->keys($pattern));
        }
        if(empty($pattern)&&isset($this->redisHash[$this->sn]->table)){
            return $this->redisHash[$this->sn]->del($this->keys($this->redisHash[$this->sn]->table.":*"));
        }
        return false;
    }
